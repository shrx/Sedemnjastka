...
<table class="posts">
    ...
    <tbody>
        <tr class="even">
            <td class="author">
                <p><strong>${h.link_to(h.literal(c.quote.post.user.nick_name), url('user', id=c.quote.post.user.id), class_='elita')}</strong></p>
                % if c.quote.post.avatar:
                ${c.quote.post.avatar.img()}
                % endif
                <p>Sporočil: ${c.quote.post.user.num_of_posts}</p>
                <p><strong>Napisano:</strong></p>
                <p>${h.fdt(c.quote.post.created_at)}</p>
            </td>
            <td>
